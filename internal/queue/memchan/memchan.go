...
// queue.Publisher[T] and queue.Consumer[T].
...
func (q *Queue[T]) Publish(ctx context.Context, ev T) error {
	select {
	case q.ch <- ev:
		return nil
	default:
...
func (q *Queue[T]) Consume(ctx context.Context, fn func(context.Context, T) error) error {
	for {
		select {
		case <-ctx.Done():
			return ctx.Err()
