...
func (r *retryBroker) PlaceOrder(ctx context.Context, variety string, params broker.OrderParams) (broker.OrderResponse, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.callCount++
	if r.callCount <= r.failCount {
		return broker.OrderResponse{}, r.errToReturn
	}
	return broker.OrderResponse{OrderID: r.successID}, nil
}

func (r *retryBroker) calls() int {
	r.mu.Lock()
	defer r.mu.Unlock()
	return r.callCount
}

func TestPool_TimeoutRetriesAndSucceeds(t *testing.T) {
	store := newFakeStore()
	b := &retryBroker{
		failCount:   2,
		errToReturn: context.DeadlineExceeded,
		successID:   "ORDER_RETRY_OK",
	}
	pool := worker.NewPool()
	pool.InitialBackoff = 5 * time.Millisecond
	defer pool.Shutdown()

	followerID := uuid.New()
	pool.Register(context.Background(), followerID, b, store, 4)

	job := domain.Job{FollowerOrderID: 40, FollowerID: followerID, Quantity: 10, IdempotencyTag: "tag40"}
	pool.Dispatch(followerID, job)

	waitFor(t, func() bool { _, ok := store.placedFor(40); return ok })
	call, _ := store.placedFor(40)
	if call.brokerOrderID != "ORDER_RETRY_OK" {
		t.Fatalf("brokerOrderID = %q, want %q", call.brokerOrderID, "ORDER_RETRY_OK")
	}
	if b.calls() != 3 {
		t.Fatalf("calls = %d, want 3", b.calls())
	}
}

func TestPool_TimeoutRetriesUpToCapAndFails(t *testing.T) {
	store := newFakeStore()
	b := &retryBroker{
		failCount:   10,
		errToReturn: errors.New("network timeout while contacting broker"),
	}
	pool := worker.NewPool()
	pool.InitialBackoff = 5 * time.Millisecond
	pool.MaxRetries = 3
	...
	job := domain.Job{FollowerOrderID: 41, FollowerID: followerID, Quantity: 10, IdempotencyTag: "tag41"}
	pool.Dispatch(followerID, job)

	waitFor(t, func() bool { _, ok := store.failedFor(41); return ok })
	if b.calls() != 3 {
		t.Fatalf("calls = %d, want 3 (capped retries)", b.calls())
	}
}
